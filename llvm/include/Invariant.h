...
class Transform {
  public:
    ...
    void gep(llvm::GetElementPtrInst *GEP, llvm::Value *LastCond,
             llvm::Value *NewCond);
  ...
  private:
    ...
    /// Keep track of the last accessed index for each memory-related value
    /// (array, pointer, etc).
    llvm::DenseMap<llvm::Value *, llvm::Value *> LastM;
