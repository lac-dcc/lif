...
#define GETuint32_t(pt) buf_get_be32(pt)
#define PUTuint32_t(ct, st) buf_put_be32(ct, st)
...
/* rotation right shift 1uint8_t */
#define CAMELLIA_RR8(x) (((x) >> 8) + ((x) << 24))
/* rotation left shift 1bit */
#define CAMELLIA_RL1(x) (((x) << 1) + ((x) >> 31))
/* rotation left shift 1uint8_t */
...
static const uint32_t camellia_sp1110[256] __attribute__((aligned(64))) = {
...
static const uint32_t camellia_sp0222[256] __attribute__((aligned(64))) = {
...
static const uint32_t camellia_sp3033[256] __attribute__((aligned(64))) = {
...
static const uint32_t camellia_sp4404[256] __attribute__((aligned(64))) = {
...
void camellia_setup128(const unsigned char *key, uint32_t *subkey) {
     ...
     */
    kll = GETuint32_t(key);
    klr = GETuint32_t(key + 4);
    krl = GETuint32_t(key + 8);
    krr = GETuint32_t(key + 12);
    /**
