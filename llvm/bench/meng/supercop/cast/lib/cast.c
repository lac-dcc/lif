...
const uint64_t CAST_S_table0[256] __attribute__((aligned(64))) = {
...
const uint64_t CAST_S_table1[256] __attribute__((aligned(64))) = {
    0x1f201094, 0xef0ba75b, 0x69e3cf7e, 0x393f4380, 0xfe61cf7a, 0xeec5207a,
    0x55889c94, 0x72fc0651, 0xada7ef79, 0x4e1d723,  0xd55a63ce, 0xde0436ba,
...
const uint64_t CAST_S_table2[256] __attribute__((aligned(64))) = {
...
const uint64_t CAST_S_table3[256] __attribute__((aligned(64))) = {
...
const uint64_t CAST_S_table4[256] __attribute__((aligned(64))) = {
...
const uint64_t CAST_S_table5[256] __attribute__((aligned(64))) = {
...
const uint64_t CAST_S_table6[256] __attribute__((aligned(64))) = {
...
const uint64_t CAST_S_table7[256] __attribute__((aligned(64))) = {
...
#define E_CAST(n, key, L, R, OP1, OP2, OP3)                                    \
    {                                                                          \
        ...
        i = key[n * 2 + 1];                                                    \
        t = ROTL(t, i);                                                        \
        L ^= (((((*(uint64_t *)((unsigned char *)CAST_S_table0 +               \
                                ((t >> C_2) & C_M)) OP2 *                      \
                  (uint64_t *)((unsigned char *)CAST_S_table1 +                \
                               ((t << C_3) & C_M))) &                          \
                 0xffffffffL) OP3 *                                            \
                (uint64_t *)((unsigned char *)CAST_S_table2 +                  \
                             ((t >> C_0) & C_M))) &                            \
               0xffffffffL) OP1 *                                              \
              (uint64_t *)((unsigned char *)CAST_S_table3 +                    \
                           ((t >> C_1) & C_M))) &                              \
             0xffffffffL;                                                      \
    }
...
#define E_CAST(n, key, L, R, OP1, OP2, OP3)                                    \
    {                                                                          \
        ...
        u &= C_M;                                                              \
        v &= C_M;                                                              \
        t = *(uint64_t *)((unsigned char *)CAST_S_table0 + u);                 \
        u = w >> C_0;                                                          \
        t = (t OP2 * (uint64_t *)((unsigned char *)CAST_S_table1 + v)) &       \
            0xffffffffL;                                                       \
        v = w >> C_1;                                                          \
        u &= C_M;                                                              \
        v &= C_M;                                                              \
        t = (t OP3 * (uint64_t *)((unsigned char *)CAST_S_table2 + u) &        \
             0xffffffffL);                                                     \
        t = (t OP1 * (uint64_t *)((unsigned char *)CAST_S_table3 + v) &        \
             0xffffffffL);                                                     \
        L ^= (t & 0xffffffff);                                                 \
    }
#else
...
#pragma unroll
    ...
    // set the loop limit to 16 (see main).
    // ================= end :: Isochronous pass ================ //
    /* for (i = 0; i < len; i++) x[i] = data[i]; */
    for (i = 0; i < 16; i++)
        if (i < len) x[i] = data[i];
...
void CAST_decrypt(const CAST_KEY *key, uint64_t *data) {
    uint64_t l, r, t;
    const uint64_t *k;
