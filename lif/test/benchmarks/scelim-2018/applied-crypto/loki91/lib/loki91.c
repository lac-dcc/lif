...
char P[32] = {31, 23, 15, 7, 30, 22, 14, 6, 29, 21, 13, 5, 28, 20, 12, 4,
...
int32_t f(int32_t r, int32_t k); /* declare LOKI function f */
short s(int32_t i);              /* declare LOKI S-box fn s */
...
void enloki(loki_ctx *c, unsigned char *b) {
    size_t i;
    int32_t L, R; /* left & right data halves */
#ifdef LITTLE_ENDIAN
...
int32_t f(int32_t r, /* Data value R(i-1) */ int32_t k /* Key K(i) */) {
...
short mult8(short a, short b, /* operands for multiply */
    ...
    /*     if (b & 01) product ^= a; /\* add multiplicand if LSB of b set *\/ */
    /*     a <<= 1;                  /\* shift multiplicand one place *\/ */
    /*     if (a >= SIZE) a ^= gen;  /\* and modulo reduce if needed *\/ */
    /*     b >>= 1;                  /\* shift multiplier one place *\/ */
    /* } */

    // ==================================================================
    // || LIF CHANGES: The original function involves a loop with only ||
    // || secret-dependent exits. We thus modified it so we can use    ||
    // || this benchmark in our experiments. Estimating an upper bound ||
    // || is basically what Constantine does (with the help of runtime ||
    // || information). In this case, though, the upper bound does not ||
    // || depend on any secret, and thus it is safe.                   ||
    // ==================================================================
    for (short i = SHRT_MAX; i >= 0; i--) {
        if (b == 0) continue;
        if (b & 01) product ^= a; /* add multiplicand if LSB of b set */
        a <<= 1;                  /* shift multiplicand one place */
        if (a >= SIZE) a ^= gen;  /* and modulo reduce if needed */
