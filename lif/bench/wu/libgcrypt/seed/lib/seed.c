...
static const uint32_t SS0[256] __attribute__((aligned(64))) = {
    ...
    0x22022220, 0x38083038, 0x34c4f0f4, 0x2787a3a4, 0x0545414,  0x0c4c404c,
...
static const uint32_t SS1[256] __attribute__((aligned(64))) = {
...
static const uint32_t SS2[256] __attribute__((aligned(64))) = {
...
static const uint32_t SS3[256] __attribute__((aligned(64))) = {
...
static const uint32_t KC[NUMKC] = {
...
int do_setkey(const uint8_t *key, SEED_context *ctx, const unsigned keylen) {
    ...
    union wordbuf t0, t1;
    uint32_t *keyout = ctx->keyschedule;
    int i;
    ...
    x1 = GETuint32_t(key);
    x2 = GETuint32_t(key + 4);
    x3 = GETuint32_t(key + 8);
    x4 = GETuint32_t(key + 12);
...
// static int
// seed_setkey (void *context, const uint8_t *key, const unsigned keylen)
// {
...
void do_encrypt(const SEED_context *ctx, uint8_t *outbuf,
                const uint8_t *inbuf) {
    uint32_t x1, x2, x3, x4;
    union wordbuf t0, t1;

    x1 = GETuint32_t(inbuf);
    x2 = GETuint32_t(inbuf + 4);
    x3 = GETuint32_t(inbuf + 8);
    x4 = GETuint32_t(inbuf + 12);
    ...
    PUTuint32_t(outbuf, x3);
    PUTuint32_t(outbuf + 4, x4);
    PUTuint32_t(outbuf + 8, x1);
    PUTuint32_t(outbuf + 12, x2);
}

// static unsigned int
// seed_encrypt (void *context, uint8_t *outbuf, const uint8_t *inbuf)
// {
//   SEED_context *ctx = context;
...
// static unsigned int
// seed_decrypt (void *context, uint8_t *outbuf, const uint8_t *inbuf)
// {
//   SEED_context *ctx = context;
