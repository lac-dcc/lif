...
static const uint64_t T0[256] __attribute__((aligned(64))) = {
...
static const uint64_t T1[256] __attribute__((aligned(64))) = {
...
static const uint64_t T2[256] __attribute__((aligned(64))) = {
...
static const uint64_t T3[256] __attribute__((aligned(64))) = {
...
static const uint64_t T4[256] __attribute__((aligned(64))) = {
...
static const uint64_t T5[256] __attribute__((aligned(64))) = {
...
static const uint64_t T6[256] __attribute__((aligned(64))) = {
...
static const uint64_t T7[256] __attribute__((aligned(64))) = {
...
static const uint64_t c[KHAZAD_ROUNDS + 1] __attribute__((aligned(64))) = {
...
int khazad_setkey(const uint8_t *in_key, struct khazad_ctx *ctx,
    ...
    K2 = ((uint64_t)(key[0]) << 32) | (key[1]);
    K1 = ((uint64_t)(key[2]) << 32) | (key[3]);

// printf("K1 is: %llu", (unsigned long long int)K1);
// printf("K1 is: %llu", (unsigned long long int)K2);
/* setup the encrypt key */
#pragma unroll
    for (r = 0; r <= KHAZAD_ROUNDS; r++) {
        ctx->E[r] = T0[(int)(K1 >> 56)] ^ T1[(int)(K1 >> 48) & 0xff] ^
                    T2[(int)(K1 >> 40) & 0xff] ^ T3[(int)(K1 >> 32) & 0xff] ^
                    T4[(int)(K1 >> 24) & 0xff] ^ T5[(int)(K1 >> 16) & 0xff] ^
                    T6[(int)(K1 >> 8) & 0xff] ^ T7[(int)(K1)&0xff] ^ c[r] ^ K2;
        K2 = K1;
        K1 = ctx->E[r];
    }
...
#pragma unroll
    for (r = 1; r < KHAZAD_ROUNDS; r++) {
        ...
        ctx->D[r] = T0[(int)T7[(int)(K1 >> 56)] & 0xff] ^
                    T1[(int)T7[(int)(K1 >> 48) & 0xff] & 0xff] ^
                    T2[(int)T7[(int)(K1 >> 40) & 0xff] & 0xff] ^
                    T3[(int)T7[(int)(K1 >> 32) & 0xff] & 0xff] ^
                    T4[(int)T7[(int)(K1 >> 24) & 0xff] & 0xff] ^
                    T5[(int)T7[(int)(K1 >> 16) & 0xff] & 0xff] ^
                    T6[(int)T7[(int)(K1 >> 8) & 0xff] & 0xff] ^
                    T7[(int)T7[(int)(K1)&0xff] & 0xff];
    }
...
void khazad_crypt(const uint64_t roundKey[KHAZAD_ROUNDS + 1],
    ...
    int r;
    uint64_t state;
    state = (*src) ^ roundKey[0];
#pragma unroll
    for (r = 1; r < KHAZAD_ROUNDS; r++) {
        state = T0[(int)(state >> 56)] ^ T1[(int)(state >> 48) & 0xff] ^
                T2[(int)(state >> 40) & 0xff] ^ T3[(int)(state >> 32) & 0xff] ^
                T4[(int)(state >> 24) & 0xff] ^ T5[(int)(state >> 16) & 0xff] ^
                T6[(int)(state >> 8) & 0xff] ^ T7[(int)(state)&0xff] ^
                roundKey[r];
    }
